...
### 1-3. RTMPS 接続確立の観測性（不正キー時の FFmpeg 挙動）
**目的**: 制御プレーンが「送出が本当に YouTube に届いているか」を即時に判定できるかを確定する（spec §4.3.1 R1「即時検知」の土台）。サイレント失敗（キーが無効なのに配信中のつもりで走り続ける）を排除できるかが問い。

**検証内容**:
- **不正／期限切れストリームキー**を渡したときの FFmpeg の挙動を観測する:
  - 即エラー終了するか（非0 exit code + stderr の RTMP handshake 失敗）か、それとも無言で再接続を試み続けるか
  - 失敗が観測できる場合、そのシグナル（exit code / stderr のエラーパターン）を R1 検知のトリガとして使えるか
- **正キー**での接続成立を観測する手段を確定する:
  - FFmpeg 側: `Output #0, flv, to 'rtmps://...'` 到達と `frame= … bitrate=` の進行（送出継続）を成立の証左とできるか
  - YouTube 側: Live 管理画面が「受信中」になるまでの遅延
- （任意）到達性が切れた場合（キー revoke・ネットワーク断）に FFmpeg がどう振る舞うか
